...
app.post('/tasklists/:tasklistId/tasks', async (request, response) => {
  const { taskName, marka, nomer } = request.body;
  ...
  tasklists[tasklistId].tasks.push([taskName, marka, nomer]);
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
...
app.put('/tasklists/:tasklistId/tasks/:taskId', async (request, response) => {
  const { newTaskName, newNomer, newMarka } = request.body;
  const tasklistId = Number(request.params.tasklistId);
  ...
  tasklists[tasklistId].tasks[taskId] = [newTaskName, newNomer, newMarka];
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
