  ...
  static async addTask({
    ...
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks`,
      {
        ...
        body: JSON.stringify({ taskName, marka, nomer })
      }
    );
  ...
  static async editTask({
    ...
    const result = await fetch(
      `http://localhost:4321/tasklists/${tasklistId}/tasks/${taskId}`,
      {
        ...
        body: JSON.stringify({ newTaskName, newNomer, newMarka })
      }
    );
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите колонку:');
    const marka = prompt('Введите марку авто:');
    const nomer = prompt('Введите рег. номер авто:');
    ...
    try {
      ...
      this.addTask(newTaskName, marka, nomer);
    } catch (error) {
      console.error('ERROR', error);
    }
  };

   addTask = (taskName, marka, nomer) => {
    document.querySelector(`#${this.tlID} ul`)
      .appendChild(
        this.renderTask({
          taskID: `${this.tlID}-T${this.tasks.length}`,
          taskName: [taskName, marka, nomer]
        })
      );
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новый номер колонки', oldTaskName[0]);
    const newNomer = prompt('Введите новый гос.номер', oldTaskName[1]);
    const newMarka = prompt('Введите новую марку', oldTaskName[2]);
    ...
    try {
      ...
      document.querySelector(`#${taskID} span`)
        .innerHTML = newTaskName + ',' + newNomer + ',' + newMarka;
    } catch (error) {
      console.error('ERROR', error);
    }
  };
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Авто '${taskName}' будет удалено. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить авто';
